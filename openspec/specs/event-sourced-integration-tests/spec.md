# event-sourced-integration-tests

## Purpose

Defines the integration- and E2E-level test guarantees for the event-sourced storage layer: a real git-backed `IEventGitContext`, real-git merge/squash replay behaviour, migration against real legacy fixtures, and commit-order tiebreaking of parallel-root change files. These requirements ensure the event-sourcing model is exercised against real git and real fixture data, not only in-memory stubs.
